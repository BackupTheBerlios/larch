...
class PopupInfo:
    """This is designed for running in the background thread. It allows
    an information window to be popped up, and later removed.
    """
    def __init__(self):
        mainWindow.idle_call(self.popup_cb, text, title)

    def popup_cb(self, text, title):
        self.popup = gtk.MessageDialog(None,
                gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT,
                gtk.MESSAGE_INFO, gtk.BUTTONS_NONE,
                title)
        self.popup.format_secondary_markup(text)
        self.popup.set_title("larchin")
        self.popup.show()

# Is it conceivable that the popup has not yet been created when this is called?
    def drop(self):
        mainWindow.idle_call(self.popup.destroy)
