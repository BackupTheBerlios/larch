# selpart_gui.py - gui handler for the mount-point selection stage
#
...
import re
    ...
    class Selp_gui:
        ...
        def __init__(self):
            ...
            # Initialize the list of mount-point suggestions
            self.listMP = mainWindow.widget("liststoreMP")
            for mp in install.mountpoints:
                self.listMP.append([mp, True])

            # Set visibility filter on mount-point suggestions
            filter = builder.get_object("treemodelfilterMP")
            filter.set_visible_column(1)

            # Initialize the list of available file-system types
            self.listFS = mainWindow.widget("liststoreFS")
            for fs in install.filesystems:
                self.listFS.append([fs])
