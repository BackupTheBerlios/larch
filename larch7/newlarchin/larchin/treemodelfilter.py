...
class TreeModelFilterExample:
    ...
    def __init__(self):
        ...
        self.filterstore = gtk.ListStore(str, bool)
        self.combostore = gtk.ListStore(str, bool)
        for i in [["c1", True], ["c2", True], ["c3", False]]:
            self.combostore.append(i)
        ...
        self.treeview.columns[3] = gtk.TreeViewColumn('Combo')

        # add bug data
        self.states = []
        for line in bugdata.split('\n'):
            l = line.split()
            self.liststore.append([int(l[0]), l[1], ' '.join(l[2:]), 'None'])
        ...
        for n in range(4):
            ...
            if n<3:
                ...
                self.treeview.columns[n].cell = crcombo
        ...
        crcombo.set_property("model", self.combostore)
